...
# define ERR_IS_DIR 6
# define ERR_EXPORT_NOT_VALID_INDENTIFIER 7	
# define ERR_UNSET_NOT_VALID_INDENTIFIER 8	
...
void				join_env(const char *key, const char *value,
						t_data *data, int to_join);
...
int					is_pipe(char c, int inside_double_quotes,
						int inside_signle_quotes);
...
char				*free_and_join(char *s1, char *variable_name,
						int origin_len, int *result_index);
...
void				move_ptr_to_redir(char **input, int *redir,
						t_quotes_status *qt);
int					move_ptr_to_filename(char **input, int redir,
						t_quotes_status *qt, t_data *data);
void				move_ptr_to_endfilename(char **input, t_quotes_status *qt);
int					alloc_new_redir(t_redirection **begin,
						t_redirection **red_list, int redir);
char				*move_ptr_and_alloc(char **input, int redir,
						t_quotes_status *qt, t_data *data);
...
int					add_export(t_env **head, const char *name,
						const char *value);
